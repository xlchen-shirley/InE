...
#IR_files = sorted(get_img_file(r"test_img/cloudy/ir"))
IR_files = sorted(get_img_file(r"MSRS_CDD/ir"))
#VIS_files   = sorted(get_img_file(r"test_img/cloudy/vi"))
VIS_files   = sorted(get_img_file(r"MSRS_CDD/vi"))

assert len(IR_files) == len(VIS_files)
h5f = h5py.File(os.path.join('.\\data',
                                 data_name+'_1_'+str(img_size)+"_stride_"+str(stride)+'.h5'),
                    'w')
h5_ir = h5f.create_group('ir_patchs')
h5_vis = h5f.create_group('vis_patchs')
h5_vis_c = h5f.create_group('vis_c_patchs')
train_num=0
for i in tqdm(range(len(IR_files))):
        ...
        I_VIS_C_Patch_Group = Im2Patch(I_VIS_C, img_size, stride)
        ...
        for ii in range(I_IR_Patch_Group.shape[-1]):
            ...
            bad_VIS_C = is_low_contrast(I_VIS_C_Patch_Group[0,:,:,ii])
            # Determine if the contrast is low
            if not (bad_IR or bad_VIS or bad_VIS_C):
                avl_IR= I_IR_Patch_Group[0,:,:,ii]  #  available IR
                avl_VIS= I_VIS_Patch_Group[0,:,:,ii]
                avl_VIS_C = I_VIS_C_Patch_Group[:,:,:,ii]
                avl_IR=avl_IR[None,...]
                avl_VIS=avl_VIS[None,...]
                avl_VIS_C = avl_VIS_C[None,...]
                ...
                h5_vis_c.create_dataset(str(train_num), data=avl_VIS_C,
                                      dtype=avl_VIS_C.dtype, shape=avl_VIS_C.shape)
                train_num += 1        

h5f.close()

with h5py.File(os.path.join('data',
                                 data_name+str(img_size)+"_stride_"+str(stride)+'.h5'),"r") as f:
    for key in f.keys():
